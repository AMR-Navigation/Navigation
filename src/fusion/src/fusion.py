...
class Fusion:
		...
		for angles, coords in self.laserarcs:
			...
			for classification, angle_x, angle_y in self.detectionarcs:
				...
				if min_angle - 0.30 <= angle_x <= max_angle + 0.30:
					matched_objects.append((classification, coords, self.directions[i], angle_x))
					matched = True
					break  # Once matched, break the inner loop
			if not matched: 	# run through previous detections as well
				for classification, point, direction, angle in self.prev_matched_objects:
					if min_angle - 0.30 <= angle <= max_angle + 0.30:

						# Save it with an updated angle
						matched_objects.append((classification, point, self.directions[i], getanglefrompoint(point.x,point.y,self.x,self.y)))
						break
			i+=1
	...
	def fuse(self):
		...
		self.prev_matched_objects = deepcopy(matched_objects)
		...
		for classification, coord, direction, _ in matched_objects:
